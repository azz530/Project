...
export default {
  ...
  methods: {
    ...
    login() { //登录
      this.$refs.login_reset.validate((valid)=>{
        if(!valid) return ;
        // let loginData = this.$http.post("url",{}); 发起登录请求
        if(this.loginForm.username){ //登录弹窗
          this.$message.success('登录成功');
        } else {
          this.$message.error('登录失败');
        }
        // 获取后端传的token 验证token进行登录  
        // window.sessionStorage.setItem('token',); 将token存储到sessionStorage
